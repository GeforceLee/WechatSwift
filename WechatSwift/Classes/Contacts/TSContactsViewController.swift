    ...
    func fetchContactList() {
        ...
        if jsonObject != JSON.null {
            ...
            if let startArray = jsonObject["data"][0].arrayObject where startArray.count > 0 {
                ...
                tempList.sortUsingSelector(#selector(ContactModel.compareContct(_:)))
                self.sortedkeys.append("★")
...
extension TSContactsViewController: UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier(TSContactTableViewCell.identifier) as!  TSContactTableViewCell
